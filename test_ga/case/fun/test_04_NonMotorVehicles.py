...
class Test_MotorVehicles():
	...
	@allure.title("非机动车信息")
	@pytest.mark.order(4)
	def test_nonNotorVehicle(self):
		NonMotorInfo = NonMotorListObj().dumps(NonMotorVehicleListObject())
		u_id = str(uuid.uuid1())
		r_redis.set(u_id, NonMotorInfo)
		uuid_ope(name=self.test_nonNotorVehicle.__name__, uuid=u_id)
		header = {
			"User-Identify": "11010800001190000001"
		}
		with allure.step('step1:添加非机动车采集信息'):
			r = request.post(url=nonNotorVehicle, headers=header, data=NonMotorInfo)
			print('------------')
			import json
		...
		with allure.step('step2:修改非机动车信息'):
			r = request.put(url=nonNotorVehicle, headers=header, data=NonMotorInfo)
			if r.get('status_code', None) == None:
				assert 0 == 200, print(r)
			else:
				assert r.get('status_code', None) == 200, print(r)
		with allure.step('step3:查询非机动车采集信息'):
			time.sleep(5)
			key_value(json.loads(r_redis.get(u_id)), 'NonMotorVehicleID')
			r = request.get(url=nonNotorVehicle + '?(NonMotorVehicles.NonMotorVehicleID={})'.format(l[0]), headers=header)
			assert type(r) != None, print(r)
			if r.get('status_code', None) == None:
				assert 0 == 200, print(r)
			else:
				assert r.get('status_code', None) == 200, print(r)
		with allure.step('step4:删除非机动车信息'):
			time.sleep(5)
			r = request.delete(url=nonNotorVehicle + '?IDList={}'.format(l[0]), headers=header)
			if r.get('status_code', None) == None:
				assert 0 == 200, print(r)
			else:
				assert r.get('status_code', None) == 200, print(r)
