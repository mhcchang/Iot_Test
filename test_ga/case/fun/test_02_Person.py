...
class Test_Person():
	...
	def test_person(self):
		personInfo = PersonListObj().dumps(PersonListObject())
		u_id = str(uuid.uuid1())
		r_redis.set(u_id, personInfo)
		uuid_ope(name=self.test_person.__name__, uuid=u_id)
		header = {
			"User-Identify": "11010800001190000001"
		}
		with allure.step('step1:添加人员采集信息'):
			r = request.post(url=person, headers=header, data=personInfo)
			print('------------')
			import json
		...
		with allure.step('step2:修改人员信息'):
			r = request.put(url=person, headers=header, data=personInfo)
			if r.get('status_code', None) == None:
				assert 0 == 200, print(r)
			else:
				assert r.get('status_code', None) == 200, print(r)
		with allure.step('step3:查询人员采集信息'):
			time.sleep(5)
			key_value(json.loads(r_redis.get(u_id)), 'PersonID')
			r = request.get(url=person + '?(Person.PersonID={})'.format(l[0]), headers=header)
			assert type(r) != None, print(r)
			if r.get('status_code', None) == None:
				assert 0 == 200, print(r)
			else:
				assert r.get('status_code', None) == 200, print(r)
		with allure.step('step4:删除人员信息'):
			time.sleep(5)
			r = request.delete(url=person + '?IDList={}'.format(l[0]), headers=header)
			if r.get('status_code', None) == None:
				assert 0 == 200, print(r)
			else:
				assert r.get('status_code', None) == 200, print(r)
