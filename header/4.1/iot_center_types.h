...
typedef enum IotDeviceConfigCommand
{
     ...
     *     "channelNo" : 0,     // 设备通道号，从0开始
     *     "profileType" : 0,   // 通道码流类型：0-主码流，1-子码流，2-第二子码流
     *     "list" : [12, 9]     // 支持的分辨率类型列表，值参见 @ref IotVideoResolutionType
     * }
     */
     ...
     *     "resolution" : 12,       // 分辨率类型，值参见 @ref IotVideoResolutionType
     ...
     *     "channelNo" : 0,             // 设备通道号，从0开始
     *     "profileType" : 0,           // 通道码流类型：0-主码流，1-子码流，2-第二子码流
     *     "resolution" : 9,            // 分辨率类型（部分类型设备会和视频共用此参数，此时此值可能不生效），值参见 @ref IotVideoResolutionType
...
{
    int64_t timestamp;                  ///< PTS时间戳，正值，单位：1/90000秒
    uint32_t seq;                       ///< 序号，从0开始，顺序递增
...
typedef struct IotServerNodeInfo
{
    ...
    uint16_t device_num;                ///< 管理的设备总数
    uint16_t stream_in_num;             ///< 接入流总数
    uint16_t stream_out_num;            ///< 转发流总数
