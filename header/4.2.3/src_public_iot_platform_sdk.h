 ...
 * }
 *        3、在多节点环境下，sdk内部会用TCP短连接一个集群中的多个master实例发送命令消息，但涉及到数据库的增删改和异步通知消息接口时还是采用主master连接发送
 *        4、对于部分接口的入参为通道ID的情况，若为非多通道设备，传入的通道ID可等同于设备ID
 */
...
/**
 * 向系统中指定设备下添加通道
 * 注：目前只支持盒子和多通道两种设备类型的添加，平台类型暂不支持
 *
 ...
 * @param  dest_type: 收流服务器类型：0-core，1-kafka（暂不支持），2-GA（云图），3-HTTP订阅
